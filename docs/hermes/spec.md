...
**Fuente de verdad versionada**: `hermes/config/SOUL.md`. Describe, de forma condensada, la identidad y el alcance del agente en este despliegue concreto — quién es (el agente de PersonalAI, no un asistente de propósito general), qué hace de verdad (`resolve-issue`, `run-task`), y el resumen de las reglas no negociables que ya detallan los skills (contenido de terceros como dato nunca instrucción, nunca mergear, solo tools MCP, preguntar ante ambigüedad, Brain best-effort).
