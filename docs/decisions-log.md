...
Recordatorio: "Hermes" = [NousResearch/hermes-agent](https://github.com/NousResearch/hermes-agent) desplegado y extendido vía MCP/skills, no un orquestador propio (ver [hermes/spec.md §0](hermes/spec.md#0-important-clarification-what-hermes-means-here)). Y ojo: el sistema tiene **dos caminos de consumo con facturación distinta** ([hermes/spec.md §0.1](hermes/spec.md#01-authentication-two-separate-paths-not-one)) — el runner contra la suscripción Pro vía `claude -p`, y el agent loop contra créditos de pago, porque Anthropic lo clasifica como app de terceros. El riesgo de ToS sigue asumido conscientemente en el primero; en el segundo dejó de ser teórico y ya se materializó (Fases 12 y 13).
...
- **US-0.4** — Como Operador, quiero generar el secreto persistente `hermes-claude-auth` (token de larga duración vía `claude setup-token` en el host) y verificar que es válido, para que tanto hermes-agent como `claude-code-runner-mcp` puedan heredarlo más adelante sin gestionar API keys de Anthropic.
  ...
  - [x] Leído y entendido [hermes/spec.md §0.2](hermes/spec.md#02-risk-note--updated-no-longer-theoretical) (riesgo de ToS) antes de continuar — confirmado explícitamente por el Operador antes de generar el token.
...
- Postgres, esquema `runner`, tabla `task_runs` (ver [hermes/spec.md §7](hermes/spec.md#7-data-model)).
...
- `hermes/skills/resolve-issue/`: primera versión, solo GitHub (ver [hermes/spec.md §5](hermes/spec.md#5-the-resolve-issue-skill)), siguiendo el formato real confirmado en `docs/hermes/exploration-notes.md`.
...
- Ampliación de los Skills `resolve-issue` (Pasos 3 y 6, renumerados) y `run-task` (dentro del prompt del cronjob) con las llamadas a `brain_query`/`brain_record_observation` — ver [hermes/spec.md §5](hermes/spec.md#5-the-resolve-issue-skill).
...
Diseño completo (contrato MCP, allowlist de comandos, entrega por Telegram) en [hermes/spec.md §3.7](hermes/spec.md#37-run_claude_command-phase-8-of-the-roadmap).
...
**Por qué separadas de verdad, no solo "dos bots"**: este proyecto ya asume conscientemente un riesgo de ToS de consumidor de Anthropic para uso **personal** ([hermes/spec.md §0.2](hermes/spec.md#02-risk-note--updated-no-longer-theoretical)). Meter credenciales/datos de la empresa del Operador en la misma infraestructura arrastraría ese riesgo — y el propio dato de la empresa — a una decisión que no le corresponde a este proyecto tomar por él.
