...
**Fase cerrada (2026-08-27)**, las cinco user stories con evidencia real: camino feliz (US-14.1, `MYAI-8`/`MYAI-10`, PRs #35 y #36 confirmados con `gh`), rechazo por repo ausente (US-14.2, `MYAI-9`), cron real disparado por el `gateway` (US-14.3), prueba de inyección hostil resistida (US-14.4, `MYAI-11`, proyecto Jira intacto y sin PR fantasma), y el estado de Jira sincronizado con la etiqueta en las tres ramas — éxito, rechazo por repo y rechazo por inyección (US-14.6, añadida a petición del Operador tras notar que el tablero no reflejaba `hermes:done`).
