...
Roles usados en las stories: **Operador** (yo, dueño único del sistema), **Hermes** (la instancia de hermes-agent desplegada, actuando como agente), **Sistema** (comportamiento automático sin actor humano), **Reclutador/cliente** (consumidor externo del portfolio, solo aparece en la Fase 7).
...
- **US-0.1** — Como Operador, quiero un monorepo con `apps/brain`, `apps/brain-mcp`, `apps/claude-code-runner-mcp` y `packages/shared` scaffolded con pnpm workspaces + TypeScript, para tener una base común antes de escribir lógica de negocio.
  - [ ] `pnpm install && pnpm build` funciona sin errores en los cuatro paquetes (vacíos o con un "hello world" tipado).
  - [ ] ESLint + Prettier configurados y compartidos entre paquetes.
  - [ ] CI (GitHub Actions) corre build + typecheck + test en cada push y está en verde.
- **US-0.2** — Como Operador, quiero `docker-compose.dev.yml` levantando Postgres con `pgvector` en local, para desarrollar Brain sin depender del VPS.
  - [ ] `docker compose -f docker-compose.dev.yml up` deja Postgres accesible en local con la extensión `vector` instalada.
...
- `claude setup-token` en el host + creación del volumen Docker nombrado.

### Definition of Done

Monorepo instalable con CI verde, Postgres+pgvector disponible en local, hermes-agent corriendo en local en modo CLI, y la sesión Pro compartida generada y verificada — con el riesgo de ToS explícitamente aceptado por el Operador antes de avanzar a la Fase 2/3.

---

## Fase 1 — Brain MVP (sin consolidación todavía)

**Objetivo**: poder ingestar una nota de texto y recuperarla por similitud semántica vía una API HTTP interna. Explícitamente **no** es todavía un "company brain" (falta la capa de consolidación, Fase 6) — el spec debe dejar esto claro para no vender de más en el portfolio.

**Depende de**: Fase 0.

### User stories

- **US-1.1** — Como Operador, quiero un endpoint/CLI para ingestar manualmente un documento de texto (nota markdown, descripción de PR pegada a mano) como `RawEvent`, para empezar a poblar Brain sin depender de conectores automáticos.
...
- **US-1.2** — Como Sistema, quiero generar y almacenar el embedding de cada `RawEvent` ingestado en pgvector, para poder hacer búsqueda por similitud más adelante.
  - [ ] Cada `RawEvent` ingestado genera un embedding (proveedor configurable) y se persiste junto al texto.
- **US-1.3** — Como Operador, quiero consultar `POST /v1/query` con una pregunta en lenguaje natural y recibir los fragmentos más relevantes por similitud semántica, para validar que la recuperación básica funciona antes de invertir en consolidación.
  - [ ] La respuesta incluye los `k` fragmentos más similares (k configurable) con su score.
  - [ ] Documentado explícitamente en el spec que esto es "vector store simple", no observations consolidadas (evita confundir esta fase con el producto final).

### Tareas técnicas

- `apps/brain/src/ingestion`: normalización a `RawEvent` + endpoint/CLI de ingesta manual.
- `apps/brain/src/retrieval`: similarity search sobre pgvector, sin entidad/temporal/grafo todavía (eso es parte de fases posteriores del propio spec de Brain, no de este roadmap).
- Sin MCP todavía — API HTTP interna únicamente (`brain-mcp` llega en la Fase 4).

### Definition of Done

Puedo pegarle una nota por API/CLI y preguntarle algo relacionado, y me devuelve el fragmento relevante por similitud. Sin consolidación, sin MCP, sin conectores automáticos — y el README/spec de Brain dice esto explícitamente.

---

## Fase 2 — `claude-code-runner-mcp` en solitario
...
- **US-2.1** — Como Operador, quiero invocar la tool MCP `run_coding_task(repo, taskTitle, taskDescription, ...)` desde un cliente MCP de prueba, para validar el ciclo completo antes de conectarlo a hermes-agent.
  ...
  - [ ] El contenedor hereda la sesión de `hermes-claude-auth` (montada read-only), sin recibir ninguna `ANTHROPIC_API_KEY`.
...
- **US-2.2** — Como Sistema, quiero comprobar la validez de la sesión de Claude Code antes de lanzar cada contenedor, para devolver `needs_human_input` de forma inmediata si la sesión expiró o fue revocada, en vez de lanzar un contenedor que fallará igualmente.
...
- **US-2.3** — Como Operador, quiero que el contenedor de ejecución no tenga acceso al socket de Docker del host ni red libre, para limitar el radio de impacto de un prompt malicioso proveniente de una issue de terceros.
...
- **US-2.4** — Como Operador, quiero un límite de tareas concurrentes/por hora configurable, para no agotar la ventana de 5h/semanal de la cuenta Pro compartida con hermes-agent.
...
- Chequeo de sesión (`claude /status` o equivalente) antes de cada `docker run`.
...
## Fase 3 — Desplegar hermes-agent + Skill `resolve-issue` (solo GitHub)
...
- **US-3.1** — Como Operador, quiero desplegar hermes-agent en el VPS vía Docker Compose con el wrapper que hace que su backend de modelo Anthropic invoque `claude -p` en vez del Agent SDK con API key, para reutilizar la misma sesión Pro compartida en el chat y en la ejecución de tareas.
  - [ ] hermes-agent responde en modo chat (CLI o gateway) usando la sesión de `hermes-claude-auth`, sin `ANTHROPIC_API_KEY` configurada.
  - [ ] Verificado que ambos componentes (hermes-agent y `claude-code-runner-mcp`) comparten el mismo volumen de sesión sin conflictos.
- **US-3.2** — Como Operador, quiero registrar el GitHub MCP oficial y `claude-code-runner-mcp` en la configuración de hermes-agent, para que el agente pueda listar issues y delegar la ejecución sin conectores propios.
  - [ ] `hermes mcp` lista ambos servidores como conectados y healthy.
  - [ ] Las credenciales de GitHub usadas son un PAT fine-grained (o GitHub App) scoped solo a los repos elegidos.
- **US-3.3** — Como Hermes, quiero un Skill `resolve-issue` que liste issues candidatas (label acordado, p. ej. `hermes`), delegue la ejecución en `run_coding_task`, y reporte el resultado en la issue original, para resolver tareas de código sin intervención humana en el camino feliz.
...
- **US-3.4** — Como Operador, quiero que el cron nativo de hermes-agent dispare el Skill cada N minutos (configurable), para no tener que ejecutar nada manualmente.
  - [ ] El Skill se ejecuta automáticamente según el cron configurado, verificado durante al menos un ciclo completo sin intervención.

### Tareas técnicas

- `hermes/docker/docker-compose.yml`: hermes-agent (imagen upstream + wrapper) + `claude-code-runner-mcp` + Postgres.
- `hermes/config/hermes.config.yaml`: registro de MCP servers, modelo, cron.
- `hermes/skills/resolve-issue/`: primera versión, solo GitHub (ver [hermes/spec.md §5](hermes/spec.md#5-el-skill-resolve-issue)).
...
**Objetivo**: la demo insignia del proyecto — Hermes resuelve una issue de forma distinta (mejor) porque consultó a Brain antes de actuar, y el resultado se escribe de vuelta como aprendizaje.
...
- **US-4.3** — Como Sistema, quiero registrar el resultado de cada ejecución (éxito, fallo, resumen) en Brain vía `brain_record_observation`, para cerrar el bucle de aprendizaje descrito en el artículo de referencia.
  - [ ] Tras cada ejecución del Skill (éxito o fallo), se crea un nuevo `RawEvent`/`Observation` en Brain de tipo `hermes_feedback`.
- **US-4.4** — Como Operador, quiero poder demostrar, con un ejemplo concreto, que una issue se resolvió mejor gracias al contexto de Brain que sin él, para tener la pieza central de la demo de portfolio.
  - [ ] Existe al menos un caso documentado (antes/después) donde el contexto de Brain cambió el resultado de la ejecución de forma verificable.
...
- **US-5.4** — Como Sistema, quiero etiquetar cada fuente de ingestion de Brain con su `source_authority` (`canonical`/`supporting`), para que la futura consolidación (Fase 6) pese correctamente los documentos de Notion/Jira ingestados como contexto.
...
## Fase 6 — Capa de consolidación real en Brain

**Objetivo**: que Brain deje de ser "un vector store con extra pasos" y cumpla de verdad las 4 propiedades de un company brain (shared, enforceable, evolving, agent-readable), vía extracción de observations, reconciliación de contradicciones y mental models.

**Depende de**: Fase 1, Fase 4 (necesita datos reales de `hermes_feedback` para que la reconciliación tenga sentido).

### User stories

- **US-6.1** — Como Sistema, quiero extraer una `Observation` estructurada (no una copia del texto) a partir de cada `RawEvent`, usando un LLM, para que Brain almacene hechos y no solo chunks de texto.
  - [ ] Un `RawEvent` de ejemplo (p. ej. una PR description) produce una `Observation` con `statement` y `entity` coherentes, verificable a mano.
- **US-6.2** — Como Sistema, quiero reconciliar una `Observation` nueva contra una existente sobre la misma entidad cuando se contradicen, aplicando una política recency-weighted por defecto, para que Brain no acumule hechos contradictorios sin resolver.
  - [ ] Al ingestar dos `RawEvent` contradictorios sobre la misma entidad, la `Observation` más antigua queda marcada `superseded_by` la más reciente.
  - [ ] La `Observation` superseded no se borra — sigue siendo consultable en modo `historical` (ver retrieval temporal de Brain).
- **US-6.3** — Como Sistema, quiero agrupar observations relacionadas en un `MentalModel` periódicamente, para que `brain_query` devuelva un resumen sintetizado en vez de observations sueltas cuando existan suficientes.
  - [ ] Tras varias observations sobre la misma entidad, existe un `MentalModel` con un `summary` que las resume.
  - [ ] `brain_query` prioriza el `MentalModel` sobre las observations individuales cuando ambos están disponibles.
- **US-6.4** — Como Operador, quiero poder borrar todo el contexto asociado a una entidad (`DELETE /v1/observations/:entity`), para poder limpiar contexto erróneo o ya no relevante.
  - [ ] Borrar una entidad hace que `brain_query` deje de devolver sus observations/mental models.

### Tareas técnicas

- `apps/brain/src/consolidation`: extracción LLM-based, reconciliación, formación de mental models (ver [personal-brain/spec.md §4.2](personal-brain/spec.md#42-consolidation-src-consolidation)).
- Job periódico (cron interno de Brain, no del hermes-agent) para la formación de mental models.
- Endpoint de borrado por entidad.

### Definition of Done

Brain cumple las 4 propiedades del artículo de referencia a nivel personal: es una única fuente de verdad, Hermes está obligado a pasar por ella, se actualiza sola con el feedback de Hermes, y es consultable de forma estructurada — con reconciliación de contradicciones demostrable con un ejemplo real.
...
- **US-7.1** — Como Reclutador/cliente, quiero un README con una demo grabada (GIF o vídeo corto) de Hermes resolviendo una issue real end-to-end, para entender el proyecto en menos de 2 minutos sin tener que leer todos los specs.
  - [ ] Existe una grabación de un ciclo completo (issue etiquetada → PR abierto) enlazada desde el README principal.
- **US-7.2** — Como Operador, quiero métricas simples expuestas (nº de tareas resueltas, nº de observations, tasa de reconciliaciones, tasa de éxito) en un comando CLI o dashboard mínimo, para poder enseñar resultados cuantitativos, no solo una demo puntual.
  - [ ] Un comando (`pnpm stats` o similar) imprime estas métricas a partir de los datos reales de `runner.task_runs` y de Brain.
- **US-7.3** — Como Operador, quiero migrar de PAT fine-grained a un GitHub App real si el tiempo lo permite, para demostrar un modelo de permisos más robusto de cara al portfolio.
  - [ ] El GitHub App está registrado e instalado solo en los repos elegidos, sustituyendo al PAT sin romper el flujo existente.
- **US-7.4** — Como Reclutador/cliente, quiero un `docs/case-study.md` que explique las decisiones de diseño (por qué hermes-agent y no un orquestador propio, por qué la sesión Pro compartida y su riesgo asumido, por qué MCP como mecanismo de integración), para entender el razonamiento detrás del código, no solo el resultado.
  - [ ] El case study cubre explícitamente: elección de hermes-agent, arquitectura MCP, y el trade-off de riesgo de ToS de la autenticación compartida.
