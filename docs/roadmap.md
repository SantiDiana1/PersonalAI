...
1. **La tanda de verificación del Operador** — siete criterios abiertos en cuatro fases (US-5.5, US-6.1, US-6.4, US-7.5, US-14.1/14.2/14.3) que no necesitan una línea de código: son mensajes reales por Telegram, una nota de voz y un ticket de Jira etiquetado. Llevaban bloqueados desde que cayó el agent loop y **hoy ya se pueden hacer**. Van primero porque cierran hasta cuatro fases por el coste de una tarde. Checklist en [`docs/hermes/verificacion-operador.md`](hermes/verificacion-operador.md).
...
- **US-14.7** — Como Operador, quiero que el cron de Jira esté acotado a un proyecto, para que una etiqueta `hermes` puesta en otro proyecto del site no le llegue.
  ...
  - [x] **Verificado con una ejecución real tras el fix**: cron disparado de nuevo, la llamada real a `jira_get` usó `project = MYAI AND labels = hermes AND statusCategory != Done` (leído del tool call real de la sesión, no inferido), devolvió `issues: []` porque no había candidatas pendientes en `MYAI`, y terminó en silencio — los 15 tickets de `WEB` ni se consultaron. Sus etiquetas quedan intactas (no se tocaron): siguen siendo el mismo hallazgo mecánicamente correcto (`hermes` + `repo:` bien formado) que ahora simplemente el cron no ve, pendiente de que el Operador decida si quitarlas hasta que el repo exista o dejarlas para cuando llegue su fase.
...
**Fase cerrada (2026-08-27)**, las siete user stories con evidencia real: camino feliz (US-14.1, `MYAI-8`/`MYAI-10`, PRs #35 y #36 confirmados con `gh`), rechazo por repo ausente (US-14.2, `MYAI-9`), cron real disparado por el `gateway` (US-14.3), prueba de inyección hostil resistida (US-14.4, `MYAI-11`, proyecto Jira intacto y sin PR fantasma), estado de Jira sincronizado con la etiqueta en las tres ramas (US-14.6), y el cron acotado a proyecto tras el hallazgo real del proyecto `WEB` (US-14.7).
