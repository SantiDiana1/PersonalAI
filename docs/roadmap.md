...
- **US-14.9** — Como Operador, quiero que una Tarea con subtareas se procese siendo consciente del desglose completo, para que no se duplique trabajo ni se pierda contenido de las subtareas.
  ...
  - [ ] **Decisión pendiente del Operador, no tomada por su cuenta**: ampliar la allowlist del proxy para incluir el registro de npm es una expansión real de superficie (un paquete de terceros puede ejecutar código arbitrario en `postinstall`, a diferencia de un `git clone`) — mismo patrón que otras decisiones de seguridad de este proyecto (SEC-2.5, SEC-6.1): se declara, no se decide sola.
...
**Bloqueante operativo, no de este proyecto**: para que una tarea real de `WEB` complete, el Operador tiene que añadir `SantiDiana1/personalWebsite` a la lista de repos del PAT fine-grained en GitHub. Sin ese paso, tanto el cron pausado (si se reanuda) como la invocación ad-hoc por Telegram fallarán igual en `run_coding_task`.
