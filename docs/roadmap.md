...
- **US-13.2** — Como Operador, quiero Ollama desplegado y hablando con hermes-agent, para tener un eslabón que no dependa de ningún proveedor externo ni cueste dinero por turno.
  - [x] **Integración escrita y verificable sin el hardware final** (el despliegue real es un Mac Mini de 16 GB todavía no disponible; se desarrolló en el portátil de trabajo del Operador). Servicio `ollama` en el compose **detrás del profile `local-llm`**, con volumen propio para los modelos, `OLLAMA_KEEP_ALIVE` (descarga el modelo de RAM tras 5 min sin uso, para no competir con Postgres/Brain/runner) y límite de memoria configurable. El profile no es precaución teórica: en la máquina de desarrollo hay ~1,9 GB libres y el contenedor moriría por OOM arrastrando la percepción de que el compose entero está roto. Verificado que `docker compose config` es válido en ambos modos y que sin el profile el servicio no aparece.
  - [x] Cadena declarada en `hermes/config/hermes.config.yaml` (`model.provider: ollama`, `fallback_providers: [anthropic]`) con el orden justificado y los comandos exactos de aplicación — ese fichero **no se copia** sobre el config real, es la declaración versionada de las claves que se fijan con `hermes config set`/`hermes fallback`.
  - [ ] **Pendiente del Mac Mini**: `ollama pull` del modelo y una respuesta real por Telegram atendida por el eslabón local. No es verificable en el portátil de desarrollo.
