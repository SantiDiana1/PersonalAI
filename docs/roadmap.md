...
**Estado**: US-8.1/US-8.2/US-8.3 cerradas, con evidencia real de la investigación, de las pruebas locales del código nuevo, y de una verificación en producción que encontró y corrigió un bug real de entrega. Único cabo suelto, no bloqueante: aplicar `ARTIFACTS_ROOT` en el despliegue real y reverificar el ciclo completo (`hermes/docker/.env.example` documenta la variable nueva).
