...
- **US-6.4** — Como Operador, quiero poder preguntarle a Hermes por su propio estado a demanda ("¿cómo estás?"), no solo recibirlo por sorpresa, para consultarlo en el momento que a mí me interese.
  ...
  - [ ] Pendiente evidencia con un mensaje de texto real del Operador por Telegram (lo verificado hasta ahora usa el disparador de cron, no el conversacional — misma lógica de skill, camino de entrada distinto).
- **US-6.5** — Como Operador, quiero poder preguntarle a Brain algo directamente por chat ("¿qué sabíamos ya sobre X?") o pedirle que recuerde algo ("anota que decidimos Y"), sin que tenga que ser un paso interno de una tarea de código.
  ...
  - [ ] Pendiente ciclo real (ingesta por chat → recuperación con una pregunta posterior) — requiere un mensaje real del Operador por Telegram.
...
**Estado: US-6.2 y US-6.3 cumplidas con evidencia real (ver checklists). US-6.4 y US-6.5 implementadas y desplegadas, con el mecanismo probado end-to-end vía cron — falta solo el disparo desde un mensaje de texto real del Operador por Telegram para cerrar sus últimos checkboxes. US-6.1 (verificación real de `run-task`↔Brain por Telegram) sigue pendiente. Ninguna de las tres requiere más código: son verificaciones que solo puede hacer el Operador enviando mensajes reales desde su cuenta de Telegram.
