...
- **US-6.5** — Como Operador, quiero poder preguntarle a Brain algo directamente por chat ("¿qué sabíamos ya sobre X?") o pedirle que recuerde algo ("anota que decidimos Y"), sin que tenga que ser un paso interno de una tarea de código.
  ...
  - [ ] Pendiente ciclo real (ingesta por chat → recuperación con una pregunta posterior) — **bug real encontrado y corregido**: en la sesión real de Fase 6, una petición de anotar algo mencionando "Brain" explícitamente usó la tool `memory` nativa de hermes-agent en vez de `brain_ingest`, así que no llegó a `brain-mcp`. Corregido con una regla de desambiguación en el skill — pendiente confirmar con un nuevo mensaje real que el fix funciona.
...
**Estado: US-6.2 cumplida con evidencia real. US-6.1/US-6.3/US-6.4/US-6.5 revisadas contra una sesión real de Telegram (09:44–12:43, 62 mensajes) que el Operador ya había usado por su cuenta — con hallazgos reales, no solo confirmaciones:**

- **Bug real encontrado (grave)**: ante "hazme un script de Python y súbelo como PR", Hermes **no cargó el skill `run-task`** — escribió el fichero y abrió el PR (#23) llamando directamente a las tools de GitHub en el turno interactivo, saltándose por completo `run_coding_task`/el aislamiento de contenedor (SEC-2.1/SEC-4.x) y la consulta/registro en Brain. `runner.task_runs` confirma 0 filas para esa tarea. US-6.1 sigue sin evidencia positiva — al contrario, tiene evidencia de que el camino esperado no se usó. Causa probable: el Paso 1 de `run-task` solo daba como ejemplos frases tipo "resuelve/arregla", no reconocía "escribe X y súbelo como PR" como el mismo tipo de petición. Corregido (trigger ampliado + regla dura contra usar tools de GitHub directamente) — pendiente una verificación real posterior al fix.
- **Bug real encontrado**: ante "escribe en Brain que me llamo Santi y tengo 25 años" (mencionando "Brain" explícitamente), Hermes usó la tool `memory` nativa de hermes-agent en vez de `ask-brain`/`brain_ingest` — la información nunca llegó a `brain-mcp`. Corregido (regla dura de desambiguación en `ask-brain/SKILL.md`) — pendiente verificación real posterior.
- **Bug real encontrado**: dos veces, ante "¿qué sabes hacer?"/"¿qué eres capaz de hacer?", Hermes respondió con una lista de capacidades genéricas de hermes-agent y una vez llegó a decir "Soy Claude Code, un asistente de IA..." — contradice directamente `SOUL.md` §Quién eres. Corregido (regla explícita "nunca te identifiques como Claude Code" añadida a `SOUL.md`) — pendiente verificación real posterior.
...
- **Pendiente de decisión del Operador, no técnica**: PR #23 (`feature/count-script`, script de contar hasta 100) quedó abierto, creado por el camino incorrecto (bypass de `run_coding_task`). Contenido inofensivo, pero no representa el flujo que debería — cerrar o mergear es decisión del Operador.

Ninguno de los tres bugs de prompt puede darse por resuelto solo con el cambio de texto — requieren una verificación real posterior (nuevos mensajes de Telegram) para confirmar que el fix realmente cambia el comportamiento del modelo.
